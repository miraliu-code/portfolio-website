...
const OBSERVATORY_DOTS: { x: number; y: number; r: number; from?: number }[] = [
          ...
          <g stroke="var(--structure)" strokeOpacity="0.25" strokeWidth="0.2">
            {OBSERVATORY_EDGES.map(([a, b]) => (
              <line
                ...
                y1={sy(OBSERVATORY_SLOTS[a].y)}
                x2={OBSERVATORY_SLOTS[b].x}
                y2={sy(OBSERVATORY_SLOTS[b].y)}
              />
            ))}
            {OBSERVATORY_DOTS.filter((dot) => dot.from !== undefined).map(
              (dot, i) => (
                <line
                  key={`t${i}`}
                  x1={OBSERVATORY_SLOTS[dot.from!].x}
                  y1={sy(OBSERVATORY_SLOTS[dot.from!].y)}
                  x2={dot.x}
                  y2={dot.y}
                />
              ),
